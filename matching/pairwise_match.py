...
def unit_layer_map(model_activs):
    ...
    return ul_map
def BB_viz(gan,gan_layers,discr,discr_layers,gan_mode,discr_mode,gan_stats,discr_stats,BB_matches,device,viz_size=10,):
    gan.eval()
    discr.eval()
#     print(gan_stats.keys())
    #### hook layers for GAN
    ...
    z,c=dataset.create_dataset(gan,gan_mode,1,1,201,device)
    z=z.float()
    c=c.float()
...
#             print(discr_activ.shape)
#             print(gan_activ.shape)
            ...
            plt.show()
#             axes[i, 0].imshow(, cmap="viridis")
#             axes[i, 0].set_title(f"GAN Layer {ganlayer}, Unit {ganunit}",fontsize="small")
            
#             # Plot discriminator activations
#             axes[i, 1].imshow(, cmap="viridis")
#             axes[i, 1].set_title(f"Discriminator Layer {discrlayer}, Unit {discrunit} , score {BB_matches[(gan_unit,discr_unit)]}",fontsize="small")
            if i==viz_size-1:
                break
